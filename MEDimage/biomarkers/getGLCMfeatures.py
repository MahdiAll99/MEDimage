...
def combine_matrices(glcm_list, merge_method, spatial_method):
    """
    Function to merge co-occurrence matrices prior to feature calculation.

    :param glcm_list: list of CooccurrenceMatrix objects.
    :param spatial_method: spatial method which determines the way co-occurrence matrices are calculated and how features are determined. One of "2d", "2.5d" or "3d".
    ...
    :return: list of one or more merged CooccurrenceMatrix objects.
        ...
        for ii_slice in np.unique(slice_id):
            ...
            if is_list_all_none(sel_matrix_list):
                ...
                use_list += [CooccurrenceMatrix(distance=glcm_list[slice_glcm_id[0]].distance, direction=None, direction_id=None,
                                                spatial_method=spatial_method, img_slice=ii_slice, merge_method=merge_method,
                                                matrix=None, n_v=0.0)]
            else:
        ...
        for ii_dir in np.unique(dir_id):
            ...
            if is_list_all_none(sel_matrix_list):
                ...
                use_list += [CooccurrenceMatrix(distance=glcm_list[dir_glcm_id[0]].distance, direction=glcm_list[dir_glcm_id[0]].direction, direction_id=ii_dir,
                                                spatial_method=spatial_method, img_slice=None, merge_method=merge_method,
                                                matrix=None, n_v=0.0)]
            else:
...
class CooccurrenceMatrix:
    ...
    def __init__(self, distance, direction, direction_id, spatial_method, img_slice=None, merge_method=None, matrix=None, n_v=None):
        """
        Initialising function for a new co-occurrence matrix
        :param distance: chebyshev distance used for the
        :param direction: direction along which neighbouring voxels are found
        :param direction_id: direction index to identify unique
         direction vectors
        :param spatial_method: spatial method used to calculate the
         co-occurrence matrix: 2d, 2.5d or 3d
        :param img_slice: corresponding slice index (only if the co-occurrence
         matrix corresponds to a 2d image slice)
        :param merge_method: method for merging the co-occurrence matrix with
         other co-occurrence matrices
        :param matrix: the actual co-occurrence matrix in sparse
    ...
    def calculate_cm_matrix(self, df_img, img_dims, dist_weight_norm):
        """
        ...
        df_cm["to_g"] = get_value(
            x=df_cm.g.values, index=df_cm.to_index.values)
        ...
        df_cm_inv = pd.DataFrame(
            {"g": df_cm.to_g, "to_g": df_cm.g, "n": df_cm.n})
    ...
    def calculate_cm_features(self, intensity_range):
        """
        Computes co-occurrence matrix features for the current co-occurrence matrix.

        :param intensity_range: range of potential discretised intensities,
        provided as a list: [minimal discretised intensity, maximal
        discretised intensity]. If one or both values
        are unknown, replace the respective values with np.nan.
        :return: data frame with values for each feature.
        """
        ...
        feat_names = ["Fcm_joint_max", "Fcm_joint_avg", "Fcm_joint_var", "Fcm_joint_entr",
                      ...
                      "Fcm_inv_diff", "Fcm_inv_diff_norm", "Fcm_inv_diff_mom", "Fcm_inv_diff_mom_norm",
                      "Fcm_inv_var", "Fcm_corr", "Fcm_auto_corr",
                      "Fcm_clust_tend", "Fcm_clust_shade", "Fcm_clust_prom", "Fcm_info_corr1", "Fcm_info_corr2"]
        ...
        if self.matrix is None:
            # Update names
            df_feat.columns += self.parse_names()
            return df_feat
        ...
        df_pi = df_pij.groupby(by="i")["pij"].agg(
            np.sum).reset_index().rename(columns={"pij": "pi"})
        df_pj = df_pij.groupby(by="j")["pij"].agg(
            np.sum).reset_index().rename(columns={"pij": "pj"})

        # Diagonal probilities p(i-j)
        df_pimj = deepcopy(df_pij)
        df_pimj["k"] = np.abs(df_pimj.i - df_pimj.j)
        df_pimj = df_pimj.groupby(by="k")["pij"].agg(
            np.sum).reset_index().rename(columns={"pij": "pimj"})

        # Cross-diagonal probabilities p(i+j)
        df_pipj = deepcopy(df_pij)
        df_pipj["k"] = df_pipj.i + df_pipj.j
        df_pipj = df_pipj.groupby(by="k")["pij"].agg(
            np.sum).reset_index().rename(columns={"pij": "pipj"})
        ...
        df_feat.loc[0, "Fcm_joint_var"] = np.sum(
            (df_pij.i - mu) ** 2.0 * df_pij.pij)

        # Joint entropy
        df_feat.loc[0, "Fcm_joint_entr"] = - \
            np.sum(df_pij.pij * np.log2(df_pij.pij))
        ...
        df_feat.loc[0, "Fcm_diff_var"] = np.sum(
            (df_pimj.k - mu) ** 2.0 * df_pimj.pimj)

        # Difference entropy
        df_feat.loc[0, "Fcm_diff_entr"] = - \
            np.sum(df_pimj.pimj * np.log2(df_pimj.pimj))
        ...
        df_feat.loc[0, "Fcm_sum_var"] = np.sum(
            (df_pipj.k - mu) ** 2.0 * df_pipj.pipj)

        # Sum entropy
        df_feat.loc[0, "Fcm_sum_entr"] = - \
            np.sum(df_pipj.pipj * np.log2(df_pipj.pipj))

        # Angular second moment
        df_feat.loc[0, "Fcm_energy"] = np.sum(df_pij.pij ** 2.0)

        # Contrast
        df_feat.loc[0, "Fcm_contrast"] = np.sum(
            (df_pij.i - df_pij.j) ** 2.0 * df_pij.pij)

        # Dissimilarity
        df_feat.loc[0, "Fcm_dissimilarity"] = np.sum(
            np.abs(df_pij.i - df_pij.j) * df_pij.pij)

        # Inverse difference
        df_feat.loc[0, "Fcm_inv_diff"] = np.sum(
            df_pij.pij / (1.0 + np.abs(df_pij.i - df_pij.j)))

        # Inverse difference normalised
        df_feat.loc[0, "Fcm_inv_diff_norm"] = np.sum(
            df_pij.pij / (1.0 + np.abs(df_pij.i - df_pij.j) / n_g))

        # Inverse difference moment
        df_feat.loc[0, "Fcm_inv_diff_mom"] = np.sum(
            df_pij.pij / (1.0 + (df_pij.i - df_pij.j) ** 2.0))

        # Inverse difference moment normalised
        df_feat.loc[0, "Fcm_inv_diff_mom_norm"] = np.sum(
            df_pij.pij / (1.0 + (df_pij.i - df_pij.j) ** 2.0 / n_g ** 2.0))

        # Inverse variance
        df_sel = df_pij[df_pij.i != df_pij.j]
        df_feat.loc[0, "Fcm_inv_var"] = np.sum(
            df_sel.pij / (df_sel.i - df_sel.j) ** 2.0)
        del df_sel
        ...
        if var_marg == 0.0:
            ...
            df_feat.loc[0, "Fcm_corr"] = 1.0 / var_marg * \
                (np.sum(df_pij.i * df_pij.j * df_pij.pij) - mu_marg ** 2.0)

        del mu_marg, var_marg

        # Autocorrelation
        df_feat.loc[0, "Fcm_auto_corr"] = np.sum(
            df_pij.i * df_pij.j * df_pij.pij)
        ...
        if hxy_2 < hxy:
            ...
            df_feat.loc[0, "Fcm_info_corr2"] = np.sqrt(
                1 - np.exp(-2.0 * (hxy_2 - hxy)))
        del hxy, hxy_2

        # Cluster tendency
        mu = np.sum(df_pi.i * df_pi.pi)
        df_feat.loc[0, "Fcm_clust_tend"] = np.sum(
            (df_pij.i + df_pij.j - 2 * mu) ** 2.0 * df_pij.pij)
        del mu

        # Cluster shade
        mu = np.sum(df_pi.i * df_pi.pi)
        df_feat.loc[0, "Fcm_clust_shade"] = np.sum(
            (df_pij.i + df_pij.j - 2 * mu) ** 3.0 * df_pij.pij)
        del mu

        # Cluster prominence
        mu = np.sum(df_pi.i * df_pi.pi)
        df_feat.loc[0, "Fcm_clust_prom"] = np.sum(
            (df_pij.i + df_pij.j - 2 * mu) ** 4.0 * df_pij.pij)
